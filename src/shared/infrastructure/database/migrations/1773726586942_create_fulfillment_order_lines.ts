...
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema
    ...
    .addColumn("order_id", "uuid", (col) => col.notNull().references("fulfillment_orders.id"))
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema.dropTable("fulfillment_order_lines").ifExists().execute();
}
